...
def load_data_and_labels_for_eval(data_file, flag='unknown'):
    """
    Loads MR polarity data from files, splits the data into words and generates labels.
    Returns split sentences and labels.
    """
    # Load data from files
    assert flag=='right' or flag=='wrong' 
    ...
    # Generate labels
    if flag=='right':
        y = [[0, 1] for _ in examples]
    if flag=='wrong':
        y = [[1, 0] for _ in examples]
    #y = np.concatenate([positive_labels, negative_labels], 0)
    return [x_text, y]
