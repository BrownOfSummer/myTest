...
def make_word_dict():
    """Read the words.txt and returns a dictionary
    ...
    # have to add single letter words tho the word list
...
def is_reducible(word, word_dict):
    """If word is reducible, returns a list of its children.
    Also adds an entry to the momo dictionary.
    ...
    """
    # if have already checked with this word, return the answer
    if word in memo:
        return memo[word]
...
def all_reducible(word_dict):
    """Check all words in the word_dict; returns a list reducible ones
    word_dict: dictionary with words as keys"""
    res = []
...
def print_trail(word):
    """Prints the sequence of word that reduces this word to the empty string.
    if there is more than one choice, it choose the first.
    word: string
    """
    if len(word) == 0:
        return
