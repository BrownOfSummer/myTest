...
def check_diff(name1, name2):
    """Computes the difference between the content of two files.
...
def compute_checksums(dirname, suffix):
    ...
    """
    names = walk(dirname) #list
    d = {}
    for name in names:
        if name.endswith(suffix):
            res, stat = compute_checksum(name)
            #checksum, _ = res.split()
            _, _, _, checksum = res.split()
    ...
    for key, names in d.iteritems():
        if len(names) > 1:
            ...
            if check_pairs(names):
                print 'And the are identical.'

if __name__=='__main__':
    d = compute_checksums(dirname='.', suffix='.pyc')
    print_duplicates(d)
